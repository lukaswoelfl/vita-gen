...
class CVRenderer(FPDF):
    ...
    def _render_experience(self, exp: Experience, prev_company: str = None):
        ...
        # Total height: Date(6) + Title(6) + Company(6) + Description + Spacing(3)
        total_height = 18 + desc_height + 3

        if self.get_y() + total_height > self.page_break_trigger:
            self.add_page()
            # Optional: re-render section title? Not required but good practice.
            # self._render_section_title("BERUFLICHE LAUFBAHN (Forts.)")
        ...
        if exp.company != prev_company:
            ...
            # Shift up slightly if it's the same company to keep roles closer
